...
public interface UserService {
    public int queryUserCount(); // 查找用户的总数
    ...
    public User getUserByUsername(String username);          // 通过用户名查找用户
    public Integer getUserByUsernameAndPassword(String username,String password); // 用户名密码是否匹配
    public List<User> queryAll();                           // 查找所有用户
    public void  deleteUserByUsername(String username);     // 通过用户名删除用户
    public Integer queryPutTagNumByUsername(String username);   // 查询一个用户贴标签的数量
    ...
    public Object getRatingGraph(String username);
